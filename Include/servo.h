...
/// @param motord Указатель на структуру тотора (структура должна существовать все время жизни сервопривода, ДОЛЖЕН БЫТЬ ПРЕИНИИЦИАЛИЗИРОВАН)
/// @param encoderd Указатель на структуру энкодера (структура должна существовать все время жизни сервопривода, ДОЛЖЕН БЫТЬ ПРЕИНИИЦИАЛИЗИРОВАН)
/// @param steps_per_rotate Количество шагов энкодера на полный оборот
/// @param rpm_min Минимальное кол-во оборотов, которое способен поддерживать серводвигатель 
/// @param pid_rpm  Указатель на структуру ПИД регулятора для управления скоростью вращения (структура должна существовать все время жизни сервопривода, ДОЛЖЕН БЫТЬ ПРЕИНИИЦИАЛИЗИРОВАН). Если не нужно, можно указать NULL, однако требуется для управления позицией
/// @param pid_position Указатель на структуру ПИД регулятора для управления положением (структура должна существовать все время жизни сервопривода, ДОЛЖЕН БЫТЬ ПРЕИНИИЦИАЛИЗИРОВАН). Если не нужно, можно указать NULL, для работы требуется наличие pid_rpm
